...
const AuthProvider = ({ children })=>{
        ...
        "AuthProvider.useEffect": ()=>{
            ...
            try {
                ...
                if (storedUser) {
                    ...
                    setUser(parsedUser);
                    setUserRole(parsedUser.email === MOCK_ADMIN_USER.email ? 'admin' : 'user');
                }
            } catch (error) {
    ...
    const updateUserSession = (newUser)=>{
        setUser(newUser);
        if (newUser) {
            const role = newUser.email === MOCK_ADMIN_USER.email ? 'admin' : 'user';
            setUserRole(role);
    ...
    const login = async (email, password)=>{
        ...
        if (email === MOCK_ADMIN_USER.email) {
            ...
            // For any other email, log in as a generic user
            userToLogin = {
                uid: `mock-uid-${Date.now()}`,
                email,
    ...
    const loginWithGoogle = async ()=>{
        ...
        // Simulate logging in with the mock regular user
        updateUserSession(MOCK_REGULAR_USER);
    };
    const logout = async ()=>{
...
_s(AuthProvider, "GgYPKy01VvonBfRWMqe5kHIchuw=", false, function() {
    return [
